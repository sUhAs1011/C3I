...
if st.session_state.analysis_triggered and resume_file and job_title_input:
    ...
    if resume_text:
        ...
        if user_skills:
            ...
            # Split text into rough sentences/blocks to find localized dates
            blocks = re.split(r'\n|\.|;', resume_text)
            for skill in user_skills:
                ...
                for block in blocks:
                    if skill_lower in block.lower():
                        # Extract years (199x to 202x)
                        years = re.findall(r'\b(199\d|200\d|201\d|202[0-6])\b', block)
                        if years:
                ...
                # Default to ~3 years old if no date explicitly found (average baseline)
                if latest_year is None:
            ...
            for skill, metrics in skill_metrics.items():
                if metrics["is_stale"]:
                    ...
                    st.info(f"📊 **{skill}** (Last seen: {metrics['year']}) - Standard Match. Temporal Weight: {metrics['weight']}x")
