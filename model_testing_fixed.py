...
def find_course_recommendations_from_dssm(skill_gap, dssm_model, client, embedding_model, top_n=5):
    ...
    # Phase 2 Upgrade: Pre-calculate Vector-Space Pedagogy Embeddings
    practical_emb = torch.tensor(embedding_model.encode(["practical hands-on lab workshop project build real-world application applied"]), dtype=torch.float32)
    theoretical_emb = torch.tensor(embedding_model.encode(["theoretical principles concepts academic lecture fundamentals science research study"]), dtype=torch.float32)
    ...
    with st.spinner("Searching for the best courses using DSSM model with SDG & Negative Vectors..."):
        for skill in skill_gap:
            ...
            for idx in top_indices:
                ...
                # --- Phase 2 Upgrade: Vector-Space Pedagogy Classification ---
                prac_sim = F.cosine_similarity(c_tensor, practical_emb).item()
                theo_sim = F.cosine_similarity(c_tensor, theoretical_emb).item()
                dominant_pedagogy = "Practical" if prac_sim > theo_sim else "Theoretical"
                
                # --- STRATEGY 3: Latent Syllabus Feature Extraction ---
                with torch.no_grad():
                    device = next(dssm_model.parameters()).device
                    base_features = dssm_model.doc_tower(c_tensor.to(device))
                    attn_weights = dssm_model.doc_attention.attention(base_features)[0].cpu().tolist()
                
                top_dims = sorted(range(len(attn_weights)), key=lambda k: attn_weights[k], reverse=True)[:3]
                latent_focus = round(sum(attn_weights[i] for i in top_dims) * 100, 2)
                ...
                rec = {
                    ...
                    "sdg": best_sdg,
                    "pedagogy": dominant_pedagogy,
                    "prac_score": prac_sim,
                    "theo_score": theo_sim,
                    "latent_dims": top_dims,
...
if st.session_state.analysis_triggered and resume_file and job_title_input:
    ...
    if resume_text:
        ...
        with st.expander("🔍 Debug: Resume Validation Details"):
            ...
            st.info(f"System Backend Output: `{validation_message}`")
            
            # --- UI Explicit Breakdown Generation for Mentor Transparency ---
            text_lower = resume_text.lower() if isinstance(resume_text, str) else ""
            
            # 1. Extraction checks
            has_resume_indicators = any(i in text_lower for i in ['resume', 'cv', 'curriculum vitae', 'professional summary', 'work experience', 'employment history', 'education', 'skills', 'certifications', 'projects', 'achievements'])
            has_contact_info = any(p in text_lower for p in ['email', 'phone', 'address', 'linkedin', 'contact'])
            has_work_section = any(p in text_lower for p in ['experience', 'employment', 'work', 'position', 'role', 'responsibilities', 'job', 'career'])
            has_education = any(p in text_lower for p in ['education', 'degree', 'university', 'college', 'school', 'diploma'])
            has_dates = bool(re.search(r'\b(19|20)\d{2}\b', resume_text if isinstance(resume_text, str) else ""))
            has_company_names = bool(re.search(r'\b(inc|corp|llc|ltd|company|corporation|technologies|solutions|systems|group|team)\b', text_lower))
            has_skills_section = any(p in text_lower for p in ['skills', 'technical skills', 'professional skills', 'competencies', 'expertise'])
            has_certifications = any(p in text_lower for p in ['certification', 'certified', 'certificate', 'license', 'accreditation'])
            has_projects = any(p in text_lower for p in ['project', 'portfolio', 'achievement', 'accomplishment', 'deliverable'])
            has_achievements = any(p in text_lower for p in ['achievement', 'accomplishment', 'result', 'outcome', 'impact', 'contribution'])
            has_responsibilities = any(p in text_lower for p in ['responsibility', 'duty', 'task', 'function', 'role', 'position'])

            raw_score = 0
            max_raw = 13
            
            # 2. Additive Rendering
            st.markdown("#### Dimensional Feature Extraction")
            if has_resume_indicators:
                raw_score += 2
                st.write(f"- Semantic Document Header: **+2 pts**")
            if has_contact_info: 
                raw_score += 1
                st.write(f"- Contact Information Vectors: **+1 pt**")
            if has_work_section: 
                raw_score += 2
                st.write(f"- Work Section Taxonomy: **+2 pts**")
            if has_education: 
                raw_score += 1
                st.write(f"- Educational Entity Markers: **+1 pt**")
            if has_dates: 
                raw_score += 1
                st.write(f"- Chronological Geometry (Years): **+1 pt**")
            if has_company_names: 
                raw_score += 1
                st.write(f"- Corporate Lexical Objects: **+1 pt**")
            if has_skills_section: 
                raw_score += 1
                st.write(f"- Extracted Skill Taxonomies: **+1 pt**")
            if has_certifications: 
                raw_score += 1
                st.write(f"- Licensing & Certifications: **+1 pt**")
            if has_projects: 
                raw_score += 1
                st.write(f"- Project Structural Objects: **+1 pt**")
            if has_achievements: 
                raw_score += 1
                st.write(f"- Impact Verbs Detected: **+1 pt**")
            if has_responsibilities: 
                raw_score += 1
                st.write(f"- Functional Responsibilities: **+1 pt**")
                
            normalized_score = int(round(10 * raw_score / max_raw))
            st.markdown("---")
            st.markdown("#### Mathematical Normalization Check")
            st.write(f"The raw scalar summation is out of an imbalanced 13-point ceiling. The model executes `round(10 * (raw / max_raw))` to normalize the structural boundary into a standard decile curve.")
            st.info(f"👉 **`round(10 * ({raw_score} / {max_raw}))`** = **{normalized_score}/10**")
            ...
            if found_irrelevant:
                ...
                st.write("**✅ No irrelevant patterns found in explicit checks**")
        ...
        if user_skills:
            st.subheader("Your Extracted Skills (Phase 3 Temporal Analysis)")
            st.markdown("We implicitly extract temporal data from your resume blocks to apply a structural decay function to older skills. This solves the identical-weighting issue (e.g. recognizing that 'NLP in 2015' holds less weight than 'NLP in 2025').")
            
            # --- Phase 3: Temporal Skill Decay Normalization ---
            import datetime
            import math
            current_year = datetime.datetime.now().year
            skill_metrics = {}
            
            # Split text into a mathematical bidirectional character window to handle bulleted resumes
            ...
            for skill in user_skills:
                ...
                # Sliding Window Character-Level Proximity Parser
                # Finds every instance of a skill and searches a 400-char window around it for a date.
                for match in re.finditer(re.escape(skill_lower), resume_lower):
                    start_idx = match.start()
                    
                    # Look 600 chars backwards (catches distant headers above the bullet point block)
                    # Look 200 chars forwards (catches inline dates "used in 2025")
                    window_start = max(0, start_idx - 600)
                    window_end = min(len(resume_text), start_idx + 200)
                    window_text = resume_text[window_start:window_end]
                    
                    # Regex extract years within this geometric proximity window
                    years = re.findall(r'\b(199\d|200\d|201\d|202[0-6])\b', window_text)
                    if years:
                        ...
                        if valid_years:
                            if latest_year is None:
                                ...
                                latest_year = max(latest_year, max(valid_years))
                
                # Default to ~3 years old if no date geometrically proximal to the skill
                is_fallback = False
                if latest_year is None:
                    latest_year = current_year - 3
                    is_fallback = True
                    
                delta_years = max(0, current_year - latest_year)
                # Mathematical exponential decay: e^(-lambda * t)
                decay_weight = math.exp(-0.15 * delta_years)
                
                skill_metrics[skill] = {
                    "year": latest_year,
                    "weight": round(decay_weight, 2),
                    "is_stale": delta_years >= 5,
                    "is_fallback": is_fallback
                }
            
            # Display Temporal UI
            for skill, metrics in skill_metrics.items():
                year_val = metrics['year']
                
                # Apply Color Coding Rules for Year Display
                if year_val >= current_year:
                    colored_year = f":green[{year_val}]"
                elif year_val >= current_year - 3:
                    colored_year = f":blue[{year_val}]"
                else:
                    colored_year = f":red[{year_val}]"
                    
                # Construct the textual string payload
                if metrics["is_fallback"]:
                    txt = f"➖ **{skill}** (No contextual date found) - Assumed Baseline (Proxy: {colored_year}). Temporal Weight: {metrics['weight']}x. *Advice: Structurally group this skill under a dated project block to improve parser calculus.*"
                elif metrics["is_stale"]:
                    txt = f"📉 **{skill}** (Last seen: {colored_year}) - Stale Skill! Temporal Weight: {metrics['weight']}x. *Advice: System detects architectural decay. A project-based syntax refresher is highly recommended for modern alignment.*"
                elif metrics["weight"] == 1.0:
                    txt = f"📈 **{skill}** (Last seen: {colored_year}) - Highly Relevant! Temporal Weight: {metrics['weight']}x"
                else:
                    txt = f"📊 **{skill}** (Last seen: {colored_year}) - Standard Match. Temporal Weight: {metrics['weight']}x"
                    
                # Apply the Background Color Box matching the specific year geometry
                if year_val >= current_year:
                    st.success(txt)  # Render deep green background box
                elif year_val >= current_year - 3:
                    st.info(txt)     # Render cool blue background box
                else:
                    st.error(txt)    # Render alert red background box
                    
            # --- Point 8: Dynamic Soft Skill Graph ---
            st.markdown("### 🧠 Dynamic Soft-Skill Enhancements")
            with st.expander("🗺️ View Technical-to-Soft Skill Enhancement Graph", expanded=False):
                dot = "digraph SoftSkills {\n"
                dot += "  rankdir=LR;\n"
                dot += "  node [shape=box, style=filled, fontname=\"Helvetica\", margin=0.2];\n"
                dot += "  User [shape=ellipse, fillcolor=lightblue, label=\"Extracted\\nProfile\"];\n"
                
                # Comprehensive semantic mappings for the dynamic psychological graph
                soft_skill_mappings = {
                    # Programming Languages
                    "python": "Logical Reasoning",
                    "java": "Object-Oriented Thinking",
                    "c++": "Memory Management Discipline",
                    "c": "Low-Level Problem Solving",
                    "javascript": "Event-Driven Thinking",
                    "typescript": "Type Safety Awareness",
                    "go": "Concurrency Mindset",
                    "rust": "Safety-Oriented Thinking",
                
                    # AI / ML / Data
                    "machine learning": "Pattern Recognition",
                    "deep learning": "Abstract Thinking",
                    "artificial intelligence": "Strategic Intelligence",
                    "ai": "Strategic Intelligence",
                    "nlp": "Language Interpretation Skills",
                    ...
                    "numpy": "Mathematical Thinking",
                    "statistics": "Probabilistic Reasoning",
                
                    # Databases
                    "sql": "Structured Thinking",
                    "mysql": "Data Organization Skills",
                    "postgresql": "Scalable Data Thinking",
                    "mongodb": "Flexible Data Modeling",
                    "database": "Information Structuring",
                    "data modeling": "Abstraction Skills",
                
                    # Web Development
                    "web development": "User-Centric Thinking",
                    "web": "User Awareness",
                    "frontend": "Visual Communication",
                    "backend": "System Structuring",
                    "fullstack": "Holistic Thinking",
                    "react": "Component-Based Thinking",
                    "angular": "Modular Architecture Thinking",
                    "vue": "Reactive Thinking",
                    "ui": "Aesthetic Judgment",
                    "ux": "Empathy Mapping",
                    "html": "Structural Design Thinking",
                    "css": "Visual Styling Sense",
                
                    # Cloud & DevOps
                    "cloud": "Scalable Systems Thinking",
                    "aws": "Infrastructure Planning",
                    "azure": "Enterprise Integration Thinking",
                    "gcp": "Distributed Systems Awareness",
                    "devops": "Collaboration Mindset",
                    "docker": "Environment Abstraction",
                    "kubernetes": "Orchestration Thinking",
                    "ci/cd": "Automation Mindset",
                    "terraform": "Infrastructure as Code Thinking",
                
                    # Software Engineering Concepts
                    "api": "Interface Design Thinking",
                    "rest": "Resource-Oriented Thinking",
                    "graphql": "Query Optimization Thinking",
                    "microservices": "Decentralized Architecture Thinking",
                    "monolith": "Centralized System Thinking",
                    "design patterns": "Reusable Solution Thinking",
                    "oop": "Encapsulation Mindset",
                    "functional programming": "Declarative Thinking",
                
                    # Security
                    "security": "Threat Awareness",
                    "cybersecurity": "Adversarial Thinking",
                    "encryption": "Confidentiality Awareness",
                    "authentication": "Identity Verification Thinking",
                    "authorization": "Access Control Judgment",
                
                    # Testing & Quality
                    "testing": "Quality Assurance Mindset",
                    "qa": "Precision Checking",
                    "unit testing": "Detail Orientation",
                    ...
                    "scrum": "Iterative Coordination",
                    "kanban": "Workflow Optimization",
                    "leadership": "Decision-Making Responsibility",
                    "mentoring": "Knowledge Transfer Ability",
                
                    # System Design
                    "system design": "Architectural Thinking",
                    "scalability": "Growth Planning",
                    "load balancing": "Traffic Distribution Thinking",
                    "distributed systems": "Consistency Tradeoff Awareness",
                
                    # Tools & Frameworks
                    "fastapi": "Efficient API Design",
                    "django": "Rapid Backend Structuring",
                    "flask": "Minimalist Design Thinking",
                    "node": "Asynchronous Thinking",
                    "express": "Lightweight Architecture Thinking",
                    "spring boot": "Enterprise System Design",
                
                    # Emerging / Advanced Tech
                    "blockchain": "Trustless System Thinking",
                    "smart contracts": "Deterministic Logic Design",
                    "web3": "Decentralized Ecosystem Thinking",
                    "iot": "Edge Connectivity Awareness",
                    "edge computing": "Latency Optimization Thinking",
                
                    # NLP / Retrieval / LLMs
                    "chromadb": "Vector Search Thinking",
                    "faiss": "Similarity Search Optimization",
                    "dssm": "Semantic Matching Reasoning",
                    "transformers": "Contextual Understanding",
                    "llm": "Generative Reasoning",
                    "ollama": "Local Model Deployment Thinking",
                    "prompt engineering": "Instruction Crafting",
                    "rag": "Knowledge Augmentation Thinking",
                
                    # Visualization & Prototyping
                    "streamlit": "Rapid Experimentation",
                    "tableau": "Data Storytelling",
                    "powerbi": "Business Insight Communication",
                    "matplotlib": "Visual Data Interpretation",
                
                    # Misc Engineering Skills
                    "git": "Version Control Discipline",
                    "github": "Collaborative Development",
                    "linux": "System-Level Thinking",
                    "bash": "Automation Scripting",
                    "shell scripting": "Process Automation Thinking",
                
                    # Soft-Tech Bridges
                    "problem solving": "Solution-Oriented Thinking",
                    "algorithms": "Stepwise Logical Thinking",
                    "data structures": "Efficient Organization Thinking"
                }
                
                # If a technical skill is extremely niche, we still want to distribute it logically
                fallback_soft_skills = [
                    "Problem Solving", "Continuous Learning", "Adaptability", 
                    "Time Management", "Resilience", "Innovative Thinking", "Collaboration"
                ]
                
                added_soft_skills = set()
                for i, skill in enumerate(user_skills[:8]): # limit to 8 top skills to avoid massive clutter
                    tech_node = f"T{i}"
                    dot += f"  {tech_node} [label=\"{skill}\", fillcolor=lightgreen];\n"
                    dot += f"  User -> {tech_node} [color=gray];\n"
                    
                    # Find mapped soft skill
                    mapped_soft = None
                    skill_lower = skill.lower()
                    
                    # 1. Exact or partial keyword match from the comprehensive dictionary
                    for kw, ss in soft_skill_mappings.items():
                        if kw in skill_lower:
                            mapped_soft = ss
                            break
                            
                    # 2. Mathematical Deterministic Fallback if not found
                    if not mapped_soft:
                        # Use character hash to pseudo-randomly (but consistently) assign a fallback trait
                        stable_hash = sum(ord(c) for c in skill_lower)
                        mapped_soft = fallback_soft_skills[stable_hash % len(fallback_soft_skills)]
                            
                    import re
                    # Create a strictly alphanumeric ID for Graphviz DOT syntax to prevent crashes from '-' or '&'
                    safe_id = re.sub(r'[^a-zA-Z0-9]', '', mapped_soft)
                    soft_node = f"S_{safe_id}"
                    
                    if mapped_soft not in added_soft_skills:
                        dot += f"  {soft_node} [label=\"{mapped_soft}\", fillcolor=plum1, shape=ellipse];\n"
                        added_soft_skills.add(mapped_soft)
                        
                    dot += f"  {tech_node} -> {soft_node} [color=purple, style=dashed, label=\" enhances\"];\n"
                            ...
                            if error:
                                ...
                                for skill, recs in recommendations.items():
                                    ...
                                    for i, rec in enumerate(sorted_recs, 1):  # Show top 3 per skill
                                        ...
                                        if "latent_dims" in rec:
                                            st.markdown(f"   🔬 **Latent Syllabus Module**: Isolating feature dims `{rec['latent_dims']}` (Focus: {rec['latent_focus']}%) to address *{skill}*")
                                        
                                        # --- Phase 2 Upgrade: Vector-Space Personality-to-Pedagogy Logic ---
                                        user_style = getattr(st.session_state, 'learning_style', "Visual (Videos & Lectures)")
                                        
                                        if "pedagogy" in rec:
                                            course_pedagogy = rec["pedagogy"]
                                            user_wants_practical = "Practical" in user_style
                                            user_wants_theoretical = "Theoretical" in user_style
                                            
                                            pedagogy_match = True
                                            if user_wants_practical and course_pedagogy != "Practical":
                                                pedagogy_match = False
                                            elif user_wants_theoretical and course_pedagogy != "Theoretical":
                                                pedagogy_match = False
                                            
                                            ped_badge = "✅ Vector Match" if pedagogy_match else "⚠️ Alternate Style"
                                            st.markdown(f"   🧠 **Learning Style ({course_pedagogy})**: {ped_badge} (Prac: {rec['prac_score']:.2f} vs Theo: {rec['theo_score']:.2f})")
                                        else:
                                            st.markdown(f"   🧠 **Learning Style**: ✅ Match ({user_style.split(' ')[0]})")
                                ...
                                # --- Phase 2 Upgrade: Master DAG Flowchart for Missing Skills ---
                                valid_skills_with_recs = {s: [r for r in recs if r.get('title') not in st.session_state.rejected_courses] 
                                                          for s, recs in recommendations.items()}
                                valid_skills_with_recs = {s: recs for s, recs in valid_skills_with_recs.items() if recs}
                                
                                if valid_skills_with_recs:
                                    with st.expander("🗺️ View Comprehensive Master Skill Roadmap (DAG)", expanded=True):
                                        dot = "digraph MasterRoadmap {\n"
                                        dot += "  rankdir=LR;\n"
                                        dot += "  node [shape=box, style=filled, fontname=\"Helvetica\", margin=0.2];\n"
                                        ...
                                        for idx, (skill, recs) in enumerate(valid_skills_with_recs.items()):
                                            top_course = recs[0]
                                            c_title = top_course.get('title', 'Course').replace('"', "'").replace("\n", " ").replace("\\", "")
                                            wrapped_title = '\\n'.join([c_title[i:i+30] for i in range(0, len(c_title), 30)])
                                            
                                            s_node = f"S{idx}"
                                            c_node = f"C{idx}"
                                            
                                            dot += f"  {s_node} [label=\"Skill Gap:\\n{skill}\", fillcolor=lightyellow];\n"
                                            dot += f"  {c_node} [label=\"{wrapped_title}\", fillcolor=lightgreen];\n"
                                            dot += f"  {s_node} -> {c_node};\n"
                                            
                                            skill_nodes.append(s_node)
                                            course_nodes.append(c_node)
                                            
                                        # Link courses sequentially to show full learning journey
                                        for idx in range(len(course_nodes)-1):
                                            dot += f"  {course_nodes[idx]} -> {course_nodes[idx+1]} [style=dashed, color=gray];\n"
