        ...
        if user_skills:
            ...
            for skill, metrics in skill_metrics.items():
                if metrics["is_fallback"]:
                    st.info(f"➖ **{skill}** (No contextual date found) - Assumed Baseline. Temporal Weight: {metrics['weight']}x")
                elif metrics["is_stale"]:
                    st.warning(f"📉 **{skill}** (Last seen: {metrics['year']}) - Stale Skill! Temporal Weight: {metrics['weight']}x")
                elif metrics["weight"] == 1.0:
                    st.success(f"📈 **{skill}** (Last seen: {metrics['year']}) - Highly Relevant! Temporal Weight: {metrics['weight']}x")
                else:
                    st.success(f"📊 **{skill}** (Last seen: {metrics['year']}) - Standard Match. Temporal Weight: {metrics['weight']}x")
