...
def generate_explanation_ollama(skill_gap, course_title, job_title):
    """Generates an explanation using a local Ollama instance."""
    
    if course_title.startswith("the sequence:"):
        prompt = (
            f"You are an AI career advisor. The user wants to become a '{job_title}'. "
            f"They are currently missing these skills: {', '.join(skill_gap)}. "
            f"Please explain in 3-4 short sentences why taking {course_title} "
            "is the most logical step-by-step roadmap for them. Focus on how the earlier "
            "courses build the foundation for the later courses in this specific sequence."
        )
    else:
        prompt = (
            f"You are an AI career advisor. The user wants to become a '{job_title}'. "
            f"They are missing these skills: {', '.join(skill_gap)}. "
            f"Explain in 2-3 short sentences why taking the course '{course_title}' "
            "will help them bridge this gap and achieve their career goal."
        )
        ...
        if dssm_model:
            ...
            if target_job_meta:
                ...
                if required_skills:
                    ...
                    st.subheader("Contextual Resume Analysis (Phase 3)")
                    with st.spinner("Segmenting resume for this specific role..."):
                        if required_skills and user_skills:
                            ...
                            # Calculate max similarity of each user skill to ANY required skill
                            ...
                            for i, skill in enumerate(user_skills):
                                if max_sims[i].item() >= 0.35: # Semantic relevance threshold
                                        ...
                                        with col1:
                                            with st.expander("✨ Why this course? (Ollama XAI)"):
                                                with st.spinner("Generating explanation..."):
                                                    explanation = generate_explanation_ollama(skill_gap, course_title, job_title)
                                ...
                                for skill, recs in recommendations.items():
                                    ...
                                    for i, rec in enumerate(sorted_recs, 1):  # Show top 3 per skill
                                        ...
                                        with col1:
                                            with st.expander("✨ Why this course? (Ollama XAI)"):
                                                with st.spinner("Generating explanation..."):
                                                    explanation = generate_explanation_ollama([skill], rec['title'], target_job_meta.get('job_title', job_title_input))
                                                    st.write(explanation)
                                        with col2:
